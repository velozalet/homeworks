...
/* ___________________________________________________B O N U S   F E A T U R E S */
    ...
    /*Save the guest list in localStorage.
        Save/Delete and Set the status to "confirmed" for the "Guest list" in localStorage
    */
    const saveGuestListToLocalStorage = () => {
        const guests = Array.from(guest_mane_ul_list.querySelectorAll("li")).map(li => {
            return {
                name: li.childNodes[0].textContent.trim(),
                confirmed: li.classList.contains("confirmed")
            };
        });
    ...
    const loadGuestListFromLocalStorage = () => {
        const guests = JSON.parse(localStorage.getItem("guests") || "[]");

        guests.forEach(guest => {
            let new_li_elem = document.createElement("li");
            let guest_name = document.createTextNode(guest.name);
            new_li_elem.appendChild(guest_name);

            createBtnFunc(new_li_elem, 'button', 'Confirm', 'btn-confirm', false);
            createBtnFunc(new_li_elem, 'button', 'X', 'btn-remove', false);

            if (guest.confirmed) {
                new_li_elem.classList.add("confirmed");
                // Hide confirm button if already confirmed
                const confirmBtn = new_li_elem.querySelector(".btn-confirm");
        ...
        // Update UI visibility and counters
        updateCntAllGuests();
        updateCntConfirmedGuests();
    ...
    loadGuestListFromLocalStorage(); // <--- call it on page load
